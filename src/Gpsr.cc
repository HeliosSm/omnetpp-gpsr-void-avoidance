...

{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // --- NUEVO: BACKPRESSURE INIT ---
        useBackpressure = par("useBackpressure");
...

{
    ...
    // --- NUEVO: BACKPRESSURE HANDLING ---
    if (packet->getKind() == BACKPRESSURE_KIND) {
        processBackpressureBeacon(packet);
        return;
    }
        // ------------------------------------
...
L3Address Gpsr::findGreedyRoutingNextHop(const L3Address& destination, GpsrOption *gpsrOption)
{
    ...
    // 1. Definir variables para el mejor vecino (Greedy) y el "Plan B" (Fallback)
    double currentDistanceToDest = (destinationPosition - selfPosition).length();

    L3Address bestGreedyNeighbor;          // Vecino más cerca del destino que yo
    double minGreedyDistance = currentDistanceToDest;

    L3Address bestFallbackNeighbor;        // Vecino cualquiera (el menos malo), por si fallamos
    double minFallbackDistance = 99999999.0; // Distancia infinita inicial

    std::vector<L3Address> neighborAddresses = neighborPositionTable.getAddresses();
    L3Address previousHop = gpsrOption->getSenderAddress(); // Evitar devolverlo inmediatamente

    for (auto& neighborAddress : neighborAddresses) {
        // A. Si está bloqueado por Backpressure, lo ignoramos totalmente
        if (useBackpressure && isBlocked(destination, neighborAddress)) {
            continue;
        }

        // B. No devolver el paquete al que me lo acaba de dar (Ping-Pong prevention)
        if (neighborAddress == previousHop) {
            continue;
        }

        Coord neighborPos = neighborPositionTable.getPosition(neighborAddress);
        double distToDest = (destinationPosition - neighborPos).length();

        // C. Buscar candidato Greedy (Estricto: debe acercarse al destino)
        if (distToDest < minGreedyDistance) {
            minGreedyDistance = distToDest;
            bestGreedyNeighbor = neighborAddress;
        }

        // D. Buscar candidato Fallback (El más cercano posible, aunque se aleje)
        // Esto sirve para "rodear" el obstáculo
        if (distToDest < minFallbackDistance) {
            minFallbackDistance = distToDest;
            bestFallbackNeighbor = neighborAddress;
        }
    }

    // --- TOMA DE DECISIÓN ---

    // OPCIÓN 1: Tenemos un vecino Greedy normal (El camino ideal)
    if (!bestGreedyNeighbor.isUnspecified()) {
        return bestGreedyNeighbor;
    }

    // OPCIÓN 2: No hay Greedy, pero activamos Backpressure y tenemos un vecino "Plan B"
    // Aquí es donde tu código actual fallaba. Ahora le permitimos usar el Plan B.
    else if (useBackpressure && !bestFallbackNeighbor.isUnspecified()) {
        EV_WARN << "VOID AVOIDANCE: No greedy neighbor. Side-stepping to " << bestFallbackNeighbor << endl;
        // Opcional: Mostrar burbuja en la simulación gráfica
        if (displayBubbles && hasGUI())
            getContainingNode(host)->bubble("Side-stepping!");

        return bestFallbackNeighbor;
    }

    // OPCIÓN 3: Realmente no hay nadie (ni bueno ni malo), o todos están bloqueados.
    else {
        // Aquí sí enviamos el Backpressure hacia atrás porque estamos totalmente atrapados
        EV_WARN << "DEAD END: Sending Backpressure to " << previousHop << endl;

        if (useBackpressure && !previousHop.isUnspecified()) {
             sendBackpressureBeacon(destination, previousHop);
        }
        return L3Address(); // Drop packet
    }
}
...
// --- NUEVO: IMPLEMENTACION DE FUNCIONES BACKPRESSURE ---

bool Gpsr::isBlocked(const L3Address& destination, const L3Address& neighbor)
{
    // Buscamos si existe una entrada para este destino
    auto it = blockedNeighbors.find(destination);
    if (it != blockedNeighbors.end()) {
        // Buscamos si el vecino está en la lista negra de este destino
        const auto& vec = it->second;
        for (const auto& blocked : vec) {
            if (blocked == neighbor) return true;
        }
    }
    return false;
}

void Gpsr::sendBackpressureBeacon(const L3Address& unreachableDest, const L3Address& targetNeighbor)
{
    EV_INFO << "Sending Backpressure Beacon to " << targetNeighbor
            << " regarding unreachable dest " << unreachableDest << endl;

    Packet *bpPacket = new Packet("BackpressureBeacon");
    bpPacket->setKind(BACKPRESSURE_KIND);

    // Creamos el payload con la dirección inalcanzable (usando BytesChunk)
    // Nota: Esto asume direcciones IPv4 (4 bytes). Para mayor robustez usar serialización propia.
    // --- CORRECCIÓN ---
        // 1. Convertimos la dirección a string (ej: "10.0.0.1")
    std::string addrStr = unreachableDest.str();

        // 2. Convertimos el string a vector de bytes para el payload
    std::vector<uint8_t> data(addrStr.begin(), addrStr.end());

        // 3. Creamos el chunk con esos datos
    const auto& chunk = makeShared<inet::BytesChunk>(data);
    bpPacket->insertAtBack(chunk);
        // ------------------

    // Configuración UDP standard para INET
    auto udpHeader = makeShared<UdpHeader>();
    udpHeader->setSourcePort(GPSR_UDP_PORT);
    udpHeader->setDestinationPort(GPSR_UDP_PORT);
    udpHeader->setCrcMode(CRC_DISABLED);
    bpPacket->insertAtFront(udpHeader);

    // Configuración L3 (Direcciones IP)
    auto addresses = bpPacket->addTag<L3AddressReq>();
    addresses->setSrcAddress(getSelfAddress());
    addresses->setDestAddress(targetNeighbor); // Unicast hacia el nodo anterior

    bpPacket->addTag<HopLimitReq>()->setHopLimit(1); // Solo un salto hacia atrás
    bpPacket->addTag<PacketProtocolTag>()->setProtocol(&Protocol::manet);
    bpPacket->addTag<DispatchProtocolReq>()->setProtocol(addressType->getNetworkProtocol());

    sendUdpPacket(bpPacket);
}

void Gpsr::processBackpressureBeacon(Packet *packet)
{
    // Extraer la dirección inalcanzable del payload
    const auto& chunk = packet->peekDataAt(B(0), packet->getDataLength());
    const auto& bytesChunk = dynamicPtrCast<const inet::BytesChunk>(chunk);

    if (bytesChunk) {
        // --- CORRECCIÓN ---
        // 1. Obtenemos los datos crudos (vector de uint8_t)
        const auto& data = bytesChunk->getBytes();

        // 2. Reconstruimos el string desde los bytes
        std::string addrStr(data.begin(), data.end());

        // 3. Parseamos el string para obtener la L3Address
        L3Address unreachableDest;
        unreachableDest.tryParse(addrStr.c_str());
        // ------------------

        // ... (El resto del código sigue igual: obtener blockedNeighbor, logs, etc.)

        // Obtener quién me envió el aviso (el vecino bloqueado)
        auto l3Ind = packet->getTag<L3AddressInd>();
        L3Address blockedNeighbor = l3Ind->getSrcAddress();

        EV_WARN << "Received Backpressure! Node " << blockedNeighbor
                << " cannot reach " << unreachableDest << ". Blocking path." << endl;

        if (displayBubbles && hasGUI()) {
            std::stringstream ss;
            ss << "Blocked " << blockedNeighbor;
            getContainingNode(host)->bubble(ss.str().c_str());
        }

        // Guardar en la lista negra
        blockedNeighbors[unreachableDest].push_back(blockedNeighbor);

        // Opcional: Podrías programar un timer para limpiar este bloqueo después de un tiempo
        // (SPEED-style soft-state). Por ahora es permanente hasta reset.
    }
